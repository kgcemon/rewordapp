...
public class RedeemAdapter extends RecyclerView.Adapter<RedeemAdapter.ViewHolder> {
    ...
    public void onBindViewHolder(@NonNull ViewHolder holder, int position) {
        ...
        Glide.with(holder.itemView.getContext()).load(WebApi.Api.IMAGES  + dataItems.get(position).getImage()).placeholder(R.drawable.placeholder).into(holder.image);
        holder.desc.setText(dataItems.get(position).getDescription());
        holder.currency.setText(dataItems.get(position).getPointvalue());
        holder.coins.setText(dataItems.get(position).getPoints());
