...
public class RewardHistoryAdapter extends RecyclerView.Adapter<RewardHistoryAdapter.ViewHolder> {
    ...
    List<RewardHistoryResponse.DataItem> dataItems;
    Context contx;

    public RewardHistoryAdapter(Context ctx, List<RewardHistoryResponse.DataItem> dataItems) {
    ...
    public void onBindViewHolder(@NonNull ViewHolder holder, int position) {
        ...
        Date inputDate = null;
        try {
            inputDate = fmt.parse(dataItems.get(position).getDate());
        } catch (ParseException e) {
            e.printStackTrace();
        }
        String mtype=dataItems.get(position).getType();
